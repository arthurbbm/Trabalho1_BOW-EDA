...
int main()//O(n^4)
{
	...
	do//O(1)
	{
		...
		switch (opcao)//O(n)
		{
			...
			case '4': exibir_bow_tr(arq_dic, arq_tra ,arq_trb);					    break;//O(n^3)
			case '5': dist_euclid(contA, contB, qtde_dic);							break;//O(n^3)
			case '6': exibir_tabela(arq_dic, contA, contB);					        break;//O(n^3)
		}
	} while (opcao != '0');//O(n)
	printf ("\n\n\n\n---> Finalizando programa.........");//O(1)
	printf ("\n________________________________________________________________________________\n\n");//O(1)
	free(contA);free(contB);//O(1)
	return 0;//O(1)
}
